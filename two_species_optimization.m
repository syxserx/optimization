...
%    0.5, 0.5, 1, 1, 0.99, 0.5, 0.1, .1, 10000;
  ...
  %obj2 Fast
   0, 0, 1, 0, 0.99, 0.5, 0.1, 1, 10000;
   0, 0, 1, 0, 0.99, 0.5, 0.1, 1, 1000;
   0, 0, 1, 0, 0.99, 0.5, 0.1, 1, 100;
...
problem = createOptimProblem('fmincon','x0',x0,'nonlcon',...
  ...
  two_species_objective_fun_fast(p,xi, beta, gamma,lambda_1,R,W),...
    'lb',lb,'ub',ub,'options',optimset);
elseif slow
problem = createOptimProblem('fmincon','x0',x0,'nonlcon',...
    @(p)unitdisk1(p),'objective',@(p) ...
    two_species_objective_fun_slow(p,xi, beta, gamma,lambda_1,R,W),...
    'lb',lb,'ub',ub,'options',optimset);
end
...
FolderDestination= strcat('fast', num2str(W(3)),...
    'plus',num2str(W(4)), ', xi=', num2str(xi),...
...
% create new file name for each run
 Filename=strcat('W=[',num2str(W(1)),',',num2str(W(2)), ']');
