function  f=two_species_objective_fun_fast(p,xi,...
    beta, gamma,lambda_1,R,W)
...
if max(A)>1||min(A)<=0
    ...
    if abs(C_1_outlet(1))>=(1-R(1))*c_10
        ...
        %  with low flux ***%
        if min(A_old)>0 && min(u)>0.1*u(1) && min(u)>0.001
        ...
        %***After pore evolution, calculate the new resistance R_m ***%
            ...
            flux_prime(1)=trapz((4./A_old.^5).*(A_new-A_old)./k)...
                /(trapz(1./A_old.^4))^2;
...
%           flux_prime(1)=(flux(2)-flux(1))/k;
            ...
            C_2_outlet_prime(1)=(C_2_outlet(2)-C_2_outlet(1))/k;
