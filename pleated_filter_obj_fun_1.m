...
%
function f =pleated_filter_obj_fun_1(p,num_pleats,R,Pec)
...
%% YX: Can we get rid of these large matrix?
bet = zeros(iterz+1,iterr+1,itery+1);
gam = zeros(iterz+1,iterr+1,itery+1);

%define function M(x)
M = @(x)  x*(x^2-2+ga*(2*log(x)-1))...
    -(1+epstar)^2/x*((1+epstar)^2-2+ga*(2*log(1+epstar)-1));
...
% Stops code when initial pore is not in the box
Y=0:dy:1;
if max(a0(Y)) > 1 || min(a0(Y))<=0
    logic = 0;
    f=1
else
...
for m=1:itertime+1
    if logic == 1
        ...
        if cavg(1) > 1-R
            ...
            J = m;
            f=2
            % [total_throughput, final_cavg initial_c] = pleated_filter(a_slope,a_intercept,num_pleats)
        end
        % -----------------------------------------------------------------
